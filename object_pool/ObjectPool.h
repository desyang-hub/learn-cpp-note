...
public:
    ...
    std::unique_ptr<T, object_pool_deleter> AcquireUnique() {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        if (!que_.empty()) {
            auto p = que_.front();
            que_.pop();
            return std::unique_ptr<T, object_pool_deleter>(p, object_pool_deleter{*this});
        }

        return nullptr;
    }

    std::shared<T> AcquireShared() {
        std::lock_guard<std::mutex> lock(mutex_);
        condition_.wait(lock, [this]{
            return !que_.empty() || is_close_;
        });
        ...
        if (!que_.empty()) {
            auto p = que_.front();
            que_.pop();
            return std::shared_ptr<T>(p, object_pool_deleter{*this});
        }
